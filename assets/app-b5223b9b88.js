...
(function () {
  ...
  function drawShip(ctx, x, y, angle, scale) {
    ...
    ctx.shadowColor = 'rgba(240,200,100,0.8)';
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.restore();
  }
  ...
  function drawMap() {
    ...
    if (bgImg.complete && bgImg.naturalWidth && !isDay) {
      const parX = (cam.x - cam.tx) * 0.02;
      const parY = (cam.y - cam.ty) * 0.02;
      mapCtx.save();
      mapCtx.filter = 'brightness(0.55) saturate(1.1) contrast(1.05)';
      mapCtx.drawImage(bgImg, parX - 20, parY - 20, w + 40, h + 40);
      mapCtx.restore();
      mapCtx.fillStyle = 'rgba(4,8,14,0.45)';
    ...
    if (layers.route) {
      ROUTES.forEach((route) => {
        ...
        const col = isMain ? 'rgba(94,184,217,0.85)' : 'rgba(180,140,90,0.5)';
        const p = isMain ? prog : 1;
        drawRouteGlow(mapCtx, pts, col, p, (isMain ? 4 : 2) / cam.scale);
      });

      const main = ROUTES.find((r) => r.id === 'main_journey');
      if (main && layers.route) {
        const shipT = playing ? (prog + animTime * 0.02) % 1 : prog;
        const sp = pointAlong(main._pts, shipT);
        const bob = Math.sin(animTime * 2.5) * 2 / cam.scale;
